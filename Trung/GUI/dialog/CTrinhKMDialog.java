...
public class CTrinhKMDialog extends javax.swing.JDialog {
    ...
    public CTrinhKMDialog() {
       ...
       txtMaCTKM.setInputVerifier(new InputVerifier() {
        ...
        txtNgayBD.setInputVerifier(new InputVerifier() {
            @Override
            public boolean verify(JComponent input){
                String ngaybd=txtNgayBD.getText().trim();
                if(!ngaybd.matches("^\\d{2}-\\d{2}-\\d{4}$")){
                    JOptionPane.showMessageDialog(CTrinhKMDialog.this, "Ngày bắt đầu phải có định dạng dd-MM-yyyy");
                    return false;
                }
                return true;
            }
        });
        txtNgayKT.setInputVerifier(new InputVerifier() {
            @Override
            public boolean verify(JComponent input){
                String ngaykt=txtNgayKT.getText().trim();
                if(!ngaykt.matches("^\\d{2}-\\d{2}-\\d{4}$")){
                    JOptionPane.showMessageDialog(CTrinhKMDialog.this, "Ngày kết thúc phải có định dạng dd-MM-yyyy");
                    return false;
                }
                return true;
            }
    ...
    public CTrinhKMDialog(BUS.CTrinhKMBUS bus,DTO.CTrinhKM ct) {
        initComponents();
        card = (CardLayout) pnlSwitch.getLayout();
        this.setTitle("Chương trình khuyến mãi");
        this.setLocationRelativeTo(null);
        ...
        txtNgayBD.setText(ct.getNgayBD());
        txtNgayKT.setText(ct.getNgayKT());
        if (ct.getHinhThucKM()) {          // KMHD
            rdoHd.setSelected(true);
            card.show(pnlSwitch, "card2");
            if (ct instanceof KMHD) {
                txtDieukienapdung.setText(String.valueOf(((KMHD) ct).getTongTienApDung()));
            }
        } else {                           // KMTour
    ...
    private boolean kiemTraThoiGian(String ngayBD, String ngayKT) {
    try {
        java.time.format.DateTimeFormatter formatter =
                java.time.format.DateTimeFormatter.ofPattern("dd-MM-yyyy");

        java.time.LocalDate bd = java.time.LocalDate.parse(ngayBD, formatter);
        java.time.LocalDate kt = java.time.LocalDate.parse(ngayKT, formatter);
    ...
    try {
        ...
        if (!kiemTraThoiGian(ngaybd, ngaykt)) {
            return;
        }
